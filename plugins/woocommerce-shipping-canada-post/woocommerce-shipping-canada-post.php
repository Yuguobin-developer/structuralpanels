...
define( 'WC_CANADA_POST_VERSION', '2.8.1' ); // WRCS: DEFINED_VERSION.
...
class WC_Shipping_Canada_Post_Init {
	...
	public function connect_canada_post() {
		...
		if ( empty( $_GET['token-id'] ) && empty( $_GET['registration-status'] ) && ! get_option( 'wc_canada_post_customer_number' ) ) {
			...
			<div id="message" class="updated">
				<p><strong><?php esc_html_e( 'Connect your Canada Post Account', 'woocommerce-shipping-canada-post' ); ?></strong> &ndash; <?php esc_html_e( 'Before you can start using Canada Post you need to register for an account, or connect an existing one.', 'woocommerce-shipping-canada-post' ); ?></p>
				<p class="submit"><a href="<?php echo esc_url( add_query_arg( 'return_url', WC()->api_request_url( 'canada_post_return' ), WC_CANADA_POST_REGISTRATION_ENDPOINT ) ); ?>" class="button-primary"><?php esc_html_e( 'Register/Connect', 'woocommerce-shipping-canada-post' ); ?></a></p>
			</div>
			<?php
		}
	}
	...
	public function upgrade_notice() {
		...
		<div class="notice notice-success is-dismissible wc-canada-post-notice">
			<p><?php echo sprintf( esc_html__( 'Canada Post now supports shipping zones. The zone settings were added to a new Canada Post method on the "Rest of the World" Zone. See the zones %shere%s ', 'woocommerce-shipping-canada-post' ), '<a href="' . esc_url( $zones_admin_url ) . '">', '</a>' ); ?></p>
		</div>
