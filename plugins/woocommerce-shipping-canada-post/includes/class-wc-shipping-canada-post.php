...
class WC_Shipping_Canada_Post extends WC_Shipping_Method {
	 ...
	 * @see https://www.canadapost-postescanada.ca/cpc/en/personal/sending/parcels/flat-rate-box.page
	 *
	 * @var array
	 */
		...
		if ( isset( $_GET['tab'] ) && 'shipping' === $_GET['tab'] && isset( $_GET['section'] ) && 'canada_post' === $_GET['section'] ) {
			...
			if ( get_option( 'wc_canada_post_customer_number' ) ) {
				echo wp_kses_post(
					wpautop(
						sprintf(
							__( 'The account <code>%s</code> is currently connected - to disconnect this account <a href="%s">click here</a>.', 'woocommerce-shipping-canada-post' ),
							esc_html( get_option( 'wc_canada_post_customer_number' ) ),
							esc_url( add_query_arg( 'disconnect_canada_post', 'true' ) )
						)
					)
				);
			}
			...
			<td class="forminp">
				<table class="canada_post_services widefat">
					<thead>
						...
						<th><?php echo sprintf( esc_html__( 'Price Adjustment (%s)', 'woocommerce-shipping-canada-post' ), esc_html( get_woocommerce_currency_symbol() ) ); ?></th>
	...
	public function validate_box_packing_field( $key ) {
		...
		$boxes_name         = isset( $_POST['boxes_name'] ) ? $_POST['boxes_name'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_outer_length = isset( $_POST['boxes_outer_length'] ) ? $_POST['boxes_outer_length'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_outer_width  = isset( $_POST['boxes_outer_width'] ) ? $_POST['boxes_outer_width']  : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_outer_height = isset( $_POST['boxes_outer_height'] ) ? $_POST['boxes_outer_height'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_inner_length = isset( $_POST['boxes_inner_length'] ) ? $_POST['boxes_inner_length'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_inner_width  = isset( $_POST['boxes_inner_width'] ) ? $_POST['boxes_inner_width'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_inner_height = isset( $_POST['boxes_inner_height'] ) ? $_POST['boxes_inner_height'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_box_weight   = isset( $_POST['boxes_box_weight'] ) ? $_POST['boxes_box_weight'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$boxes_max_weight   = isset( $_POST['boxes_max_weight'] ) ? $_POST['boxes_max_weight'] : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function validate_services_field( $key ) {
		$services         = array();
		$posted_services  = $_POST['canada_post_service']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, sanitized in the loop below.
	...
	public function calculate_flat_rate( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			for ( $i = 0; $i < $values['quantity']; $i ++ ) {
				$boxpack->add_item( wc_get_dimension( $dimensions[2], 'cm' ), wc_get_dimension( $dimensions[1], 'cm' ), wc_get_dimension( $dimensions[0], 'cm' ), wc_get_weight( $values['data']->get_weight(), 'kg' ), $values['data']->get_price() );
			}
		}
