...
add_filter("grassblade_mark_complete", function($return, $data, $statement_json, $xapi_content_id, $user_id) {
	...
	$default_percentage = 100;//empty( $statement->result->success )? 0:100;
	$percentage = isset($statement->result->score->scaled)? $statement->result->score->scaled*100:((!empty($statement->result->score->max) && isset($statement->result->score->raw))? $statement->result->score->raw*100/($statement->result->score->max - @$statement->result->score->min):$default_percentage);
