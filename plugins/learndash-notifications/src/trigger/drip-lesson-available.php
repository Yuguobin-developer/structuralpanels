...
class Drip_Lesson_Available extends Trigger {
	...
	public function maybe_dispatch_emails( $lesson_id ) {
		...
		if ( false === $this->is_shared_course() ) {
			$course_id = learndash_get_course_id( $lesson_id );
			$this->dispatching_email( $course_id, $lesson_id, $models );
		} else {
			$course_ids = $this->get_course_ids_for_shared_coursed( $lesson_id );
	 ...
	 * @param $models
	 */
	private function dispatching_email( $course_id, $lesson_id, $models ) {
		$user_ids = $this->get_users( $course_id );
		$this->log( '====Cron Start====' );
	...
	public function listen() {
		add_action( 'updated_postmeta', array( &$this, 'listen_for_lesson_update' ), 99, 4 );
